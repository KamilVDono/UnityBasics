...
namespace Odc4
{
	public class BoxCaster : MonoBehaviour, ICaster
	{
		...
		{
			return Physics.BoxCast( ray.origin, _boxHalfSize, ray.direction, out hitInfo, 
				Quaternion.Euler( _camera.transform.forward ), maxDistance, layers );
		}

		public RaycastHit[] CastAll( Ray ray, float maxDistance, int layers )
		{
			return Physics.BoxCastAll( ray.origin, _boxHalfSize, ray.direction, 
				Quaternion.Euler( _camera.transform.forward ), maxDistance, layers );
		}

		public int CastNonAlloc( Ray ray, RaycastHit[] results, float maxDistance, int layers )
		{
			return Physics.BoxCastNonAlloc( ray.origin, _boxHalfSize, ray.direction, results, 
				Quaternion.Euler( _camera.transform.forward ), maxDistance, layers );
		}
	}
}
